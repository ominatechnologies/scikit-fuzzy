...
class ControlSystemSimulation(object):
    ...
    def print_state(self):
        ...
        for v in log_state_obj.antecedents:
            print("{!s:<35} = {}"
                  .format(v, log_state_obj.dict_antecedents_input()[str(v)]))
        ...
        print(" Rules ")
        print("=======")
        rules = inner_workings["Rules"]
        for rn in rules:
            rule = rules[rn]
            for rule_number_str, r in rule[0].items():
                print("RULE #{:d}:\n  {!s}\n".format(rn, r))

            print("  Aggregation (IF-clause):")
            values = rule[1]['Aggregation (IF-clause)_membership_values']
            for cnt in range(len(values)):
                for term_full_label, term_mbs_value in values[cnt].items():
                    print("  - {:<55}: {}".format(term_full_label,
                                                  term_mbs_value))
            firings = rule[2]['Aggregation (IF-clause)_aggregate_firing']
            for r_antecedent, r_aggregate_firing in firings.items():
                print("    {!s:>54} = {}".format(r_antecedent,
                                                 r_aggregate_firing))
            print("  Activation (THEN-clause):")

            for c, c_activation in rule[3]['Activation (THEN-clause)'].items():
                print("    {!s:>54} : {}".format(c, c_activation))
            print("")
        print("")
        rule_number = {}
    ...
    def log_state(self):
        ...
        TO BE REMOVED AFTER class LogStateControlSystemSimulation(object) is
        finalized.
        """
        if next(self.ctrl.consequents).output[self] is None:
            raise ValueError("Call compute method first.")
